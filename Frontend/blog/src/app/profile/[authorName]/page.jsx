...
export default function AuthorCard({ author, className = "" }) {
    ...
    // Formats "Akinatde Temidayo" to "Akinatde-Temidayo" for clean URLs
    const formattedName = author.fullName ? encodeURIComponent(author.fullName.replace(/\s+/g, '-')) : 'User';

    // Generates the new clean vanity path with essential background variables attached as query params
    const profileLink = `/profile/${formattedName}?id=${author._id || author.id}&bio=${encodeURIComponent(author.bio || '')}&avatar=${encodeURIComponent(author.profilePicture || '')}`;
    ...
    return (
        <div className={`relative bg-[#FAFAFA] border border-[#E2E8F0] rounded-xl px-6 pb-8 pt-20 flex flex-col items-center text-center mt-12 shadow-xs transition-all hover:shadow-sm ${className}`}>
            ...
            {/* Social Media Link List Loop */}
            <div className="flex gap-3 mt-6 z-10">
                {Object.keys(SOCIAL_ICONS).map((platform) => (
                    <a
                        href={`#${platform}`}
                        key={platform}
                        onClick={(e) => e.stopPropagation()} // Keeps social click behaviors local to this button
